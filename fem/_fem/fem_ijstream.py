...
class MatrixStream(SkelAgent):
    ...
    def get_stream_from_voids(self):
        ...
        stream = IJStream.from_data_train(west.data, east.data)
    ...
    def stream_west(self):
        ...
        from_west = self.stream_from_map(
            self.westmap.nodnums1, self.westmap
        )

        from_core = self.stream_from_map(
            self.westmap.nodnums1, self.coremap
        )

        from_east = self.stream_from_map(
            self.westmap.nodnums1, self.eastmap
        )

        return IJStream.from_data_train(
            from_west.data, from_core.data, from_east.data
        )

    def stream_east(self):

        from_west = self.stream_from_map(
            self.eastmap.nodnums2, self.westmap
        )
